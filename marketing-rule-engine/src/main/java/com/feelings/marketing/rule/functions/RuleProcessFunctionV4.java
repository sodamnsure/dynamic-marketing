...
import com.feelings.marketing.rule.service.QueryRouterV4;
...
public class RuleProcessFunctionV4 extends KeyedProcessFunction<String, LogBean, ResultBean> {
    ...
    public void processElement(LogBean logBean, Context context, Collector<ResultBean> collector) throws Exception {
        ...
        if (ruleParam.getTriggerParam().getEventId().equals(logBean.getEventId())) {
            ...
            boolean b = queryRouterV4.profileQuery(logBean, ruleParam);
            if (!b) return;

            System.out.println("画像条件满足");

            // 先查询序列条件，因为序列条件比较难满足，这样就减少了查询次数类条件的次数
            boolean b1 = queryRouterV4.seqConditionQuery(logBean, ruleParam, eventState);
            if (!b1) return;

            boolean b2 = queryRouterV4.countConditionQuery(logBean, ruleParam, eventState);
            if (!b2) return;
